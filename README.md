...
That command is the setup checklist. Thirteen places, listed file by file below. When it
...
/catchup  ->  /plan  ->  plan-reviewer  ->  build  ->  /wrap
   where am I     the contract    attack it      against the      rewrite the state,
   what is stale  in a file       before it is   file, not the    delete what died
                                  built         chat
```
